...
for the rest of the day, as shown below both in terms of opportunities captured and spend over time.
...
Let's call this optimal threshold `p*`. Note how picking a larger threshold would result in not finishing the budget in full and picking a lower value would result in finishing the budget too early.
...
would build a platform that attempts to maximize **total clicks** in the system (i.e. CTR, assuming impressions are 
...
Note that as `λ` decreases, only opportunities with higher `p(click)` win the auction, 
increasing the expected CTR (`E(clicks) / impressions`) for this ad.
