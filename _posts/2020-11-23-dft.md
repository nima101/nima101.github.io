...
<b>Recap:</b> We start with a function $$F(x)$$ and pick our samples as positive and negative pairs. We only need to compute the positive samples, cutting down our problem from $$n$$ to two smaller sub-problems of size $$n/2$$. Note that the sample size is reduced in half, as well as the degree of the polynomial. If we can make this work, we have an $$O(nlogn)$$ recursive solution!
